...
@load "noise_f2_test.jld2"

data_values = Vector{Vector{Float64}}(undef, 10)

for i = 1:10
    X = data_nodes[i]
    data_values[i] = [friedman2( X[:, j] ) for j = 1:M] + data_noise[i]
end
...
for i = 1:10
    X = data_nodes_test[i]
    data_values_test[i] = [friedman2( X[:, j] ) for j = 1:1000] + data_noise_test[i]
end

bw = [4,2]

println( "Friedman 2 Approximation with U^{(4,2)} and N = [4,2]" )

err_mse = 0.0
numFreq = 0
lambda = 0
gsis = zeros( length( get_superposition_set(4,2) ) - 1 )
U = 0
...
for bw in bws 
    ...
    for i = 1:10
        ads = ANOVAapprox.approx(data_nodes[i], data_values[i], f2_active_set, bw, "cos")
...
for i = 1:100 
    ...
    dist = Normal( 0.0, 125.0 )

    X = rand(rng, 4, 200) 
    X_test = rand(rng, 4, 1000)
    y = [friedman2( X[:, i] ) for i = 1:200] + rand( rng, dist, 200 )
    y_test = [friedman2( X_test[:, i] ) for i = 1:1000] + rand( rng, dist, 1000 )

    ads = ANOVAapprox.approx(X, y, f2_active_set, bw, "cos")
    ANOVAapprox.approximate(ads, lambda = [2.0,])
    mse_vec[i] = ANOVAapprox.get_mse( ads, X_test, y_test, 2.0 )
end
