...
@load "y_test_2.jld2"
@load "y_train_2.jld2"
@load "noise_test_2.jld2"
@load "noise_train_2.jld2"
...
activeSet = copy(ANOVAapprox.get_ActiveSet( fun_approx, [0.1,0.1] )[min_res[2][2]])
...
function crossValidate( )
    ...
    for i = 1:fold 
        train = generateData( 2, 200 )
        test = generateData( 2, 1000 )
        X_train = train[1]
        y_train = train[2] + train[3]
        X_test = test[1]
        y_test = test[2] + test[3]
        fun_approx = ANOVAapprox.nperiodic_approx( X_train, complex(y_train), 2, [4, 2], active_set=f2_active_set )
        ANOVAapprox.approximate( fun_approx, max_iter=1000, lambda=[0.0,] )
        mses[i] = ANOVAapprox.get_MSE( fun_approx, X_test, complex(y_test), 0.0 )
    end
